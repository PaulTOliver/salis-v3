...
bool ui_proc_is_live(u64 pix) {
    return pix >= g_cores[g_core].pfst && pix <= g_cores[g_core].plst;
}

int ui_proc_pair(u64 pix) {
    if (pix == g_proc_selected) {
        return PAIR_SELECTED_PROC;
    } else if (ui_proc_is_live(pix)) {
...
const char *ui_proc_state(u64 pix) {
    return ui_proc_is_live(pix) ? "live" : "dead";
}
...
void ui_print_process_gene(int l, int gcol, u64 gidx, u64 mba, u64 pix, int pair) {
    ...
    wchar_t gsym[2] = { arch_gfx_symbol(byte), L'\0' };
    cchar_t cchar   = { 0 };
...
void ui_print_cell(u64 i, u64 r, u64 x, u64 y) {
    ...
    if (g_wrld_zoom == 1) {
        inst_nstr[0] = arch_gfx_symbol((u8)inst_avrg);
    } else {
...
void ui_print_wcursor_bar() {
    ...
    arch_gfx_render_mnemonic(cbyte, cmnem);
